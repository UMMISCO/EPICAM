...
public class CommonEntityHelper 
{
    ...
    public static boolean isRegionDistrictCdtHierarchyPresent(List<FieldEntity> fields)
    {
    	boolean hasRegion = false;
    	boolean hasDistrict = false;
    	boolean hasCdt = false;
        ...
        for (Iterator<FieldEntity> it = fields.iterator(); it.hasNext(); )
        {
            ...
            if ((f instanceof RelationFieldEntity) && ((RelationFieldEntity)f).getCardinality() == 1) {
            	...
            	if(rf.getEntity().getName().equals("Region"))
            		hasRegion = true;
            	else if (rf.getEntity().getName().equals("DistrictSante"))
            		hasDistrict = true;
            	else if (rf.getEntity().getName().equals("CentreDiagTrait"))
            		hasCdt = true;
            }

        }
        return hasRegion && hasDistrict && hasCdt;
