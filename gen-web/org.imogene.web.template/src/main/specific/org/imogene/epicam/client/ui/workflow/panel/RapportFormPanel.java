...
import org.imogene.epicam.client.ui.workflow.RapportEditorWorkflow;
...
/**
 * Panel that contains the Rapport Workflow
 * 
 * @author Medes-IMPS
 */
public class RapportFormPanel extends Composite {

	private static final Binder uiBinder = GWT.create(Binder.class);

	interface Binder extends UiBinder<Widget, RapportFormPanel> {
	}

	@UiField(provided = true)
	WrapperPanel wrapperPanel;
	@UiField(provided = true)
	RapportEditorWorkflow editorWorkflow;
	 ...
	 *            otherwise
	 */
	public RapportFormPanel(EpicamRequestFactory requestFactory, String entityId, RelationPopupPanel parent,
			String initField) {
		...
		if (entityId != null) {
			if (parent == null)
				editorWorkflow = new RapportEditorWorkflow(requestFactory, entityId, titleContainer);
			else {
				editorWorkflow = new RapportEditorWorkflow(requestFactory, entityId, titleContainer, parent, initField);
			}
		} else {
			if (parent == null) {
				editorWorkflow = new RapportEditorWorkflow(requestFactory, titleContainer);
			} else {
				editorWorkflow = new RapportEditorWorkflow(requestFactory, titleContainer, parent, initField);
			}
		}
	 ...
	 * @param entityId the id of the entity to be managed by the workflow. null if an entity is being created
	 */
	public RapportFormPanel(EpicamRequestFactory requestFactory, String entityId) {
